...
def dense(x, num_units, init_scale=1.0, counters={}, init=False, **kwargs):
    ...
    with tf.variable_scope(name):
        xs = x.shape.as_list()
        V = tf.get_variable(
            "V", [xs[1], num_units], tf.float32, tf.random_normal_initializer(0, 0.05)
        )
        g = tf.get_variable(
            "g", [num_units], dtype=tf.float32, initializer=tf.constant_initializer(1.0)
        )
        b = tf.get_variable(
            "b", [num_units], dtype=tf.float32, initializer=tf.constant_initializer(0.0)
        )
...
@add_arg_scope
def residual_block(
    x, a=None, conv=conv2d, init=False, dropout_p=0.0, gated=False, **kwargs
):
