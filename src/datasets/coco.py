...
class VUnetCocoDataset(Dataset):
    def __init__(
        self, coco_dir, index_p, split="train", img_shape=(256, 256), box_factor=2
    ):
    ...
    def __getitem__(self, i):
        ...
        image_path = os.path.join(self.coco_dir, self.imgs[i])
...
class COCOTrain(edu.DatasetMixin):
    def __init__(self, config):
        super(COCOTrain, self).__init__()
        self.config = config
        dataset_params = config["dataset_params"]

        self.dset = VUnetCocoDataset(**dataset_params, split="train")
...
class COCOVal(COCOTrain):
    def __init__(self, config):
        self.config = config
        dataset_params = config["dataset_params"]

        self.dset = VUnetCocoDataset(**dataset_params, split="valid")
